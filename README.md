...
## 機器人網址
- [Telegram Channel](https://t.me/HpoiInfoBig5)
- [Telegram Bot](https://t.me/hpoi_info_big5_bot)
...
- 除了`hpoi.gs`以外的其他五個`.gs`檔皆須部署為網路應用程式並取得執行url
- 所有需要的private的網址、token皆放在各檔案的前幾行，請自行部署得到這些值後填入
- `line_data_sheet.gs`為`line_data.xlsx`的指令碼編輯器，`telegram_data_sheet.gs`為`telegram_data.xlsx`的指令碼編輯器，而`hpoi_data_sheet.gs`為`hpoi_data.xlsx`的指令碼編輯器，使用方法為新建一試算表後點上方選單列的`工具>指令碼編輯器`即可打開google app script
...
> 已新增PaaS部分的code，存放於`line_notify/`及`tgbot_notify`中，使用python並搭配Flask寫成
